...
if [ -n "$CUSTOM_FILENAME" ]; then
    ...
    FILENAME_SCRIPT="$SCRIPT_DIR/generate_container_filename.py"
    if [ ! -f "$FILENAME_SCRIPT" ]; then
        echo "Error: Filename generator script not found: $FILENAME_SCRIPT"
        exit 1
    fi
    BASE_NAME=$(python3 "$FILENAME_SCRIPT" "$CONTAINER_URI")
fi
