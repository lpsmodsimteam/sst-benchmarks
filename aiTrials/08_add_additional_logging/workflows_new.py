...
def _run_cmd_capture(args, timeout=10):
    try:
        ...
        if not details:
            details.append(f"(no output, rc={result.returncode})")
        return {
        ...
        return {"ok": False, "value": "command not found", "returncode": 127}
    except subprocess.TimeoutExpired:
        return {"ok": False, "value": f"timed out after {timeout}s", "returncode": None}
    except Exception as exc:
        return {"ok": False, "value": f"error: {exc}", "returncode": None}


def _collect_environment_metadata(log_dir):
    env_keys = [
        k for k in sorted(os.environ)
        if k.startswith("SLURM_") or k.startswith("APPTAINER_") or k.startswith("SINGULARITY_") or k.startswith("E4S_")
    ]
    ...
    command_checks = {
        ...
        "e4s-cl": ["e4s-cl", "--version"],
        "srun": ["srun", "--version"],
        "scontrol": ["scontrol", "--version"],
        "lscpu": ["lscpu"],
    }

    command_outputs = {
        name: _run_cmd_capture(args)
        for name, args in command_checks.items()
    }

    git_snapshot = {
        "rev": _run_cmd_capture(["git", "rev-parse", "HEAD"]),
        "branch": _run_cmd_capture(["git", "rev-parse", "--abbrev-ref", "HEAD"]),
        "status_short": _run_cmd_capture(["git", "status", "--short"]),
    }
    ...
    return {
        ...
        "environment_variables": env_subset,
        "command_outputs": command_outputs,
        "git": git_snapshot,
    }


def _write_environment_snapshot(log_dir):
    metadata = _collect_environment_metadata(log_dir)
    output_path = os.path.join(log_dir, "environment_snapshot.json")
    with open(output_path, "w") as f:
        json.dump(metadata, f, indent=2, sort_keys=True)
    return output_path, metadata


def _collect_image_metadata(image_path):
    image_info = {
    ...
    image_info["size_bytes"] = stat_info.st_size
    image_info["mtime"] = time.strftime("%Y-%m-%dT%H:%M:%S%z", time.localtime(stat_info.st_mtime))
    return image_info
...
def set_workflow_log(log_path):
    ...
    log(f"> cd {log_dir}")
    snapshot_path, metadata = _write_environment_snapshot(log_dir)
    log(f"Environment snapshot written to {snapshot_path}")
    log(f"Host: {metadata.get('hostname', '')}")
    log(f"Platform: {metadata.get('platform', {}).get('system', '')} {metadata.get('platform', {}).get('release', '')} {metadata.get('platform', {}).get('machine', '')}")
...
def launch_and_log_sst(image, srun_args, sst_args, log_file, config_path=None, safe_display: SafeDisplay | None = None, depends_on=None):
    ...
    image_metadata = _collect_image_metadata(image)
    log(f"SST image metadata: {json.dumps(image_metadata, sort_keys=True)}")
